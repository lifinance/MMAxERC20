...
/// @title xERC20
/// @author LIFI
/// @notice forked and adapted from https://github.com/defi-wonderland/xERC20
contract xERC20 is ERC20, Ownable, IXERC20, ERC20Permit {
    ...
    function xChainTransfer(
        ...
        if (_bridgeId == MULTI_BRIDGE_ID) {
            ...
            mmaSender.remoteCall{value: msg.value}(
                _dstChainId, mmaReceiver, abi.encode(_receiver, _amount), bridgeFees, 2, new address[](0)
            );
        } else {
    ...
    function setBridgeAddress(uint256 _bridgeId, address _bridgeAddress) external {
    ...
    function setLockbox(address) public {
        revert();
    }
